...
- [ ] **🎰 Casinò: rivelare il Pokémon vinto al jackpot**: `TIER_MESSAGES.jackpot` in `components/CasinoScene.js:7` dice solo "hai catturato un Pokémon raro" in modo generico. L'id reale (Porygon `137` o Porygon2 `233`, 50/50) è deciso in `playCasino()` (`ExploreSceneContainer.js:40-55`) ma non torna mai al componente che mostra l'esito. Da far restituire l'id vinto da `playCasino()` e passarlo a `CasinoScene` per mostrare sprite+nome reale nell'esito invece del solo testo.
- [ ] **🎾 Indicatore "già catturato" nell'incontro (icona Poké Ball)**: `EncounterScene.js` riceve `onSeen`/`onCaught` come callback verso il Pokédex ma non riceve mai indietro se l'id incontrato è già presente in `state.pokedexRun` come `caught`. Il dato esiste già (hook `usePokedexState`), va solo passato come prop in più a `EncounterScene`/`PokemonPreview` per disegnare un badge Poké Ball sullo sprite quando la specie è già posseduta — stesso pattern visivo dei giochi ufficiali.
- [ ] **🏆 Più achievement facili + notifica a schermo**: oggi solo 4 in `engine/achievements.js`, tutti da run avanzata/difficile (primo Shiny, Nuzlocke completato, Rosso leggendario, tutte le 9 regioni). Da aggiungere achievement early-game (prima cattura, prima evoluzione, primo Capopalestra battuto, prima Master Ball ottenuta...). Inoltre `unlockAchievement()` restituisce `true`/`false` ma nessun componente lo intercetta oggi per notificare: da aggiungere un toast/popup che appaia subito quando torna `true`, invece di scoprirlo solo aprendo manualmente il modale Trofei.
